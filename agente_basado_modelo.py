    ...
    def posiciones_sucias(self):
        ...
        if self.matriz[self.pos_x][self.pos_y] == '1':
             for valor in range(len(self.pos_sucio_x)):
                 if (self.pos_x == self.pos_sucio_x[valor]) and (self.pos_y == self.pos_sucio_y[valor]) :
                    no_repetido = False
             if no_repetido == True :
                 self.pos_sucio_x.append(self.pos_x)
                 self.pos_sucio_y.append(self.pos_y)
